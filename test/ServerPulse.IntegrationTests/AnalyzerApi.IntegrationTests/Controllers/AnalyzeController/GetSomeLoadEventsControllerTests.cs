...
namespace AnalyzerApi.IntegrationTests.Controllers.AnalyzeController
{
    ...
    internal class GetSomeLoadEventsControllerTests : BaseIntegrationTest
    {
        private string key;
        ...
        {
            // Arrange
            var request = new GetSomeLoadEventsRequest { Key = key, NumberOfMessages = 1, StartDate = DateTime.UtcNow, ReadNew = false };
        ...
        {
            // Arrange
            var request = new GetSomeLoadEventsRequest { Key = key, NumberOfMessages = 2, StartDate = DateTime.UtcNow, ReadNew = false };
        ...
        {
            // Arrange
            var request = new GetSomeLoadEventsRequest { Key = key, NumberOfMessages = 1, StartDate = DateTime.MinValue, ReadNew = true };
        ...
        {
            // Arrange
            var request = new GetSomeLoadEventsRequest { Key = key, NumberOfMessages = 2, StartDate = DateTime.MinValue, ReadNew = true };
        ...
        {
            // Arrange
            var request = new GetSomeLoadEventsRequest { Key = key, NumberOfMessages = 10, StartDate = DateTime.MinValue, ReadNew = true };
        ...
        private static IEnumerable<TestCaseData> GetBadRequestTestCases()
        {
            yield return new TestCaseData(
                new GetSomeLoadEventsRequest { Key = "", NumberOfMessages = 1, StartDate = DateTime.MinValue, ReadNew = true }
            ).SetDescription("Invalid Key, must be not empty.");

            yield return new TestCaseData(
                new GetSomeLoadEventsRequest { Key = null!, NumberOfMessages = 1, StartDate = DateTime.MinValue, ReadNew = true }
            ).SetDescription("Invalid Key, must be not null.");

            yield return new TestCaseData(
                new GetSomeLoadEventsRequest { Key = "someRandomKey281", NumberOfMessages = 0, StartDate = DateTime.MinValue, ReadNew = true }
            ).SetDescription("Invalid NumberOfMessages, must be greater than 0.");

            var maxNumberOfMessages = 20;

            yield return new TestCaseData(
                new GetSomeLoadEventsRequest { Key = "SomeRandomKey151", NumberOfMessages = maxNumberOfMessages + 1, StartDate = DateTime.MinValue, ReadNew = true }
        ...
        public async Task GetSomeLoadEvents_InvalidRequest_ReturnsBadRequest(GetSomeLoadEventsRequest request)
        {
            // Arrange
            using var httpRequest = new HttpRequestMessage(HttpMethod.Post, "/analyze/someevents");
            httpRequest.Content = new StringContent(JsonSerializer.Serialize(request), Encoding.UTF8, "application/json");
        ...
        {
            // Arrange
            var request = new GetSomeLoadEventsRequest { Key = Guid.NewGuid().ToString(), NumberOfMessages = 1, StartDate = DateTime.MinValue, ReadNew = true };
