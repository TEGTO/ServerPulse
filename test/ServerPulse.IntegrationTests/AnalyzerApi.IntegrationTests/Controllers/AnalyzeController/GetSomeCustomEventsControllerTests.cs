...
namespace AnalyzerApi.IntegrationTests.Controllers.AnalyzeController
{
    ...
    internal class GetSomeCustomEventsControllerTests : BaseIntegrationTest
    {
        private string key;
        ...
        {
            // Arrange
            var request = new GetSomeCustomEventsRequest { Key = key, NumberOfMessages = 1, StartDate = DateTime.UtcNow, ReadNew = false };
        ...
        {
            // Arrange
            var request = new GetSomeCustomEventsRequest { Key = key, NumberOfMessages = 2, StartDate = DateTime.UtcNow, ReadNew = false };
        ...
        {
            // Arrange
            var request = new GetSomeCustomEventsRequest { Key = key, NumberOfMessages = 1, StartDate = DateTime.MinValue, ReadNew = true };
        ...
        {
            // Arrange
            var request = new GetSomeCustomEventsRequest { Key = key, NumberOfMessages = 2, StartDate = DateTime.MinValue, ReadNew = true };
        ...
        {
            // Arrange
            var request = new GetSomeCustomEventsRequest { Key = key, NumberOfMessages = 10, StartDate = DateTime.MinValue, ReadNew = true };
        ...
        private static IEnumerable<TestCaseData> GetBadRequestTestCases()
        {
            yield return new TestCaseData(
                new GetSomeCustomEventsRequest { Key = "", NumberOfMessages = 1, StartDate = DateTime.MinValue, ReadNew = true }
            ).SetDescription("Invalid Key, must be not empty.");

            yield return new TestCaseData(
                new GetSomeCustomEventsRequest { Key = null!, NumberOfMessages = 1, StartDate = DateTime.MinValue, ReadNew = true }
            ).SetDescription("Invalid Key, must be not null.");

            yield return new TestCaseData(
                new GetSomeCustomEventsRequest { Key = "someRandomKey19", NumberOfMessages = 0, StartDate = DateTime.MinValue, ReadNew = true }
            ).SetDescription("Invalid NumberOfMessages, must be greater than 0.");

            var maxNumberOfMessages = 20;

            yield return new TestCaseData(
                new GetSomeCustomEventsRequest { Key = "someRandomKey129", NumberOfMessages = maxNumberOfMessages + 1, StartDate = DateTime.MinValue, ReadNew = true }
        ...
        public async Task GetSomeCustomEvents_InvalidRequest_ReturnsBadRequest(GetSomeCustomEventsRequest request)
        {
            // Arrange
            using var httpRequest = new HttpRequestMessage(HttpMethod.Post, "/analyze/somecustomevents");
            httpRequest.Content = new StringContent(JsonSerializer.Serialize(request), Encoding.UTF8, "application/json");
        ...
        {
            // Arrange
            var request = new GetSomeCustomEventsRequest { Key = Guid.NewGuid().ToString(), NumberOfMessages = 1, StartDate = DateTime.MinValue, ReadNew = true };
