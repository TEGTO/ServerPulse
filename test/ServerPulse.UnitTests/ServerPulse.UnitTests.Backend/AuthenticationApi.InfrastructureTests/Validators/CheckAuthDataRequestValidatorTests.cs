...
namespace AuthenticationApi.Validators.Tests
{
    ...
    public class CheckAuthDataRequestValidatorTests
    {
        ...
        private static IEnumerable<TestCaseData> ValidationTestCases()
        {
            ...
            yield return new TestCaseData(
                new string('A', 257), "validPassword",
                false,
                "Login"
        ...
        public void Validate_CheckAuthDataRequestCases(string? login, string? password, bool isValid, string? errorProperty)
        {
            //Arrange
            var request = new CheckAuthDataRequest()
            {
                Login = login!,
                Password = password!
            };
