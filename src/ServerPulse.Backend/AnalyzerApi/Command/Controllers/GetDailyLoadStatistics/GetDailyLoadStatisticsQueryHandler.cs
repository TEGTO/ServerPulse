...
{
    public class GetDailyLoadStatisticsQueryHandler : IRequestHandler<GetDailyLoadStatisticsQuery, IEnumerable<LoadAmountStatisticsResponse>>
    {
        private readonly ILoadAmountStatisticsReceiver receiver;
        private readonly IMapper mapper;

        public GetDailyLoadStatisticsQueryHandler(ILoadAmountStatisticsReceiver loadAmountStatisticsReceiver, IMapper mapper)
        {
            this.receiver = loadAmountStatisticsReceiver;
            this.mapper = mapper;
        }

        public async Task<IEnumerable<LoadAmountStatisticsResponse>> Handle(GetDailyLoadStatisticsQuery command, CancellationToken cancellationToken)
        {
            var key = command.Key;
